...
The filter controls are not accessible via keyboard navigation because an `<a>` is used without an `href`. Activating the controls performs an action rather than navigation. `<a>` is the wrong tag to use here.
...
Also, when the filter controls are activated via mouse, the focus shifts to the Search section which is above the filters section.
