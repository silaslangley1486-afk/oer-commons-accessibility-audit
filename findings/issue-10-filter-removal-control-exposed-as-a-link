...
Also, when the filter's control has focus, the screen reader does not name the filter that is to be removed, nor is there any indication that selecting the control will update the search results. Screen reader users are therefore given the wrong role for the control and not enough information about what it's for.

### Impact:

Screen readers users will wrongly expect navigation when selecting the control. This can lead to confusion, accidental filter removal, and difficulty understanding why search results have changed.
